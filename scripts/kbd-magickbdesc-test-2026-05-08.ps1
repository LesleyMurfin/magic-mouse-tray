# kbd-magickbdesc-test-2026-05-08.ps1
# Smoke test: HidD_GetFeature(Col02, RID=0x47, len=2) should now succeed
# after MagicKbDesc filter binds and patches the descriptor.
# Expected output: *** SUCCESS *** [47 NN]   BATTERY = NN%
...
if ($h -eq [IntPtr]::new(-1)) { Write-Host "CreateFile FAILED err=$([Runtime.InteropServices.Marshal]::GetLastWin32Error())" -ForegroundColor Red; exit 1 }
...
if ($ok) {
    ...
    if ($err -eq 1) { Write-Host '  err=1 = ERROR_INVALID_FUNCTION (descriptor patch not applied — check driver stack has MagicKbDesc)' }
    if ($err -eq 87) { Write-Host '  err=87 = ERROR_INVALID_PARAMETER (likely buffer size mismatch)' }
    exit 1
}
