...
**Status:** v1.1 — **OUTCOME: SHIP-BLOCKED** (PATH-A v3 descriptor causes BSOD `0xD1` in `applewirelessmouse+0x9f0e` — see `docs/PATH-A-V3-BSOD-RCA.md`). Test plan retained for historical reference only. Do NOT execute any test class on user's daily driver until Phase 2 static analysis identifies a safe descriptor revision.
**Date:** 2026-05-08
...
| 20 | Track A CI-probe pre-test (post NLM-F3) | **PRE-FLIGHT for Track A bundle build.** Test whether Windows Code Integrity accepts a patched .sys with a corrupted-hash WHQL overlay + a valid testsigned catalog. If CI rejects (`STATUS_INVALID_IMAGE_HASH` or driver fails to load), the canonical pattern with overlay-intact 78424B is BLOCKED — must strip the WHQL overlay before catalog signing. **A1 (overlay-intact) is the variant shipped by both reference projects (`sbagirici/apple-magic-mouse-scroll-fix-windows`, `MagicMouse2DriversWin11x64-master`) and is the expected-pass case; A2 (overlay-stripped) is contingency only.** | Build a probe bundle with the 78424B patched .sys (overlay intact) + .cat + INF; install via `pnputil`; check `Get-WinEvent System` for CI rejection events; verify driver loads | If CI accepts: proceed with overlay-intact canonical pattern. If CI rejects: pivot to overlay-strip variant (load WHQL cert table, zero out, recompute file size, re-sign catalog) | **Track A — gate-zero** |
